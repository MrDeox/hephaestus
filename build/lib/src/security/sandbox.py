...
class SafeExecutionEnvironment:
    ...
    def execute_code(self, code: str, timeout_seconds: Optional[int] = None) -> ExecutionResult:
...
class RSISandbox:
    ...
    @trace_operation("sandbox_execute_code")
    def execute_code(
        self,
        code: str,
        ...
        result = self._execute_code_in_sandbox(code, sandbox_type, timeout_seconds)
        ...
        if (result.status in [ExecutionStatus.ERROR, ExecutionStatus.SECURITY_VIOLATION] and
            ...
            fallback_result = self._execute_code_in_sandbox(code, self.fallback_sandbox, timeout_seconds)
    ...
    def _execute_code_in_sandbox(
        ...
        try:
            if sandbox_type == SandboxType.RESTRICTED_PYTHON:
                return self.restricted_python.execute_code(code, timeout_seconds)
            
            elif sandbox_type == SandboxType.DOCKER_CONTAINER:
                if self.docker_sandbox.is_available():
                    return self.docker_sandbox.execute_code(code, timeout_seconds)
                else:
                    return ExecutionResult(
                        status=ExecutionStatus.ERROR,
                        output="",
                ...
                return self.subprocess_sandbox.execute_code(code, timeout_seconds)
